...
export class ProviderMismatchError extends Error {
  constructor(provider: Provider, sessionPath: string) {
    super(`File is not a confident ${provider} session: ${sessionPath}`);
    this.name = "ProviderMismatchError";
  }
}

export async function assertProviderSession(
  ...
  for (const r of recs) {
    ...
    if (provider === "codex") {
      if (type === "session_meta") confidence += 4;
      if (type === "tool_call" && "arguments" in o) confidence += 2;
      if (
        type === "tool_result" &&
      ...
      if (
        content.some(
          (b) =>
            obj(b).type === "tool_result" &&
            typeof obj(b).tool_use_id === "string",
        )
      )
        confidence += 3;
      if (o.hook_event_name && "tool_response" in o) confidence += 3;
    } else if (provider === "pi") {
      if (
        type === "session_start" &&
      ...
      if (
        ...
        confidence += 3;
      if (type === "branch_summary" || type === "model_change") confidence += 2;
      if (typeof o.session_id === "string" && o.session_id.startsWith("pi_"))
        confidence += 2;
    }
  }
  if (confidence < 3) throw new ProviderMismatchError(provider, sessionPath);
}
