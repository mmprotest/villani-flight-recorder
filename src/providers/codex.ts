...
export async function parseCodexSession(
  ...
  for (const r of recs) {
    ...
    if (o.hook_event_name) {
      ...
      push(
        event(
          ...
          "codex",
          c.type,
          `Codex hook: ${String(o.hook_event_name)} ${name}`,
          r.value,
          {
            timestamp: ts,
            sessionId,
    ...
    if (type === "session_meta") {
      push(
        event(
          ...
          "Session started",
          r.value,
          { timestamp: ts, sessionId, cwd, summary: String(o.model ?? "") },
        ),
      );
      continue;
    }
    if (type === "user_message") {
      push(
        event(`codex-${++n}`, "codex", "user_message", "User prompt", r.value, {
          timestamp: ts,
          sessionId,
          cwd,
    ...
    if (type === "tool_call") {
      ...
      push(
        event(
          ...
          "codex",
          c.type,
          name.toLowerCase().includes("mcp") ? "MCP tool call" : "",
          r.value,
          {
            timestamp: ts,
            sessionId,
    ...
    if (type === "tool_result") {
      const exitCode =
        typeof o.exit_code === "number" ? o.exit_code : undefined;
      push(
        event(
          `codex-${++n}`,
          "codex",
          exitCode && exitCode !== 0 ? "error" : "tool_result",
          "",
          r.value,
          {
            timestamp: ts,
      ...
      for (const f of files.length ? files : [undefined])
        push(
          event(
            ...
            "codex",
            "file_edit",
            f ? `Applied patch to ${f}` : "Applied patch",
            r.value,
            { timestamp: ts, sessionId, cwd, path: f, diff: patch },
          ),
        );
      continue;
    }
