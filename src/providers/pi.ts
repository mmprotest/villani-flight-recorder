...
export async function parsePiSession(
  ...
  for (const r of recs) {
    ...
    if (type === "session_start") {
      push(
        event(`pi-${++n}`, "pi", "session_start", "Session started", r.value, {
          timestamp: ts,
          sessionId,
          cwd,
    ...
    if (type === "session_end") {
      push(
        event(`pi-${++n}`, "pi", "session_end", "Session ended", r.value, {
          timestamp: ts,
          sessionId,
          cwd,
        }),
    ...
    if (type === "tool_call" || type === "tool_execution_start") {
      ...
      push(
        event(
          ...
          "pi",
          c.type,
          type === "tool_execution_start" ? `Pi tool started: ${name}` : "",
          r.value,
          {
            timestamp: ts,
            sessionId,
    ...
    if (type === "tool_execution_update") {
      push(
        event(
          ...
          "pi",
          "tool_result",
          `Pi tool update: ${String(o.tool_name ?? "tool")}`,
          r.value,
          {
            timestamp: ts,
            sessionId,
    ...
    if (type === "tool_execution_end") {
      ...
      const code =
        typeof res.exit_code === "number" ? res.exit_code : undefined;
      push(
        event(
          `pi-${++n}`,
          "pi",
          code && code !== 0 ? "error" : "tool_result",
          code && code !== 0
            ? `Command failed: ${lastCommand ?? String(o.tool_name ?? "tool")}`
            : `Pi tool finished: ${String(o.tool_name ?? "tool")}`,
          r.value,
          {
            timestamp: ts,
            sessionId,
    ...
    if (type === "model_change") {
      push(
        event(
          ...
          "pi",
          "tool_call",
          `Model changed: ${String(o.from)} to ${String(o.to)}`,
          r.value,
          { timestamp: ts, sessionId, cwd },
        ),
      );
      continue;
    }
