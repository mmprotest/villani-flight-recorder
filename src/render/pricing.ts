...
/** List rates in USD per million tokens. */
 ...
 */
export function matchModelRates(
  modelId: string,
  ...
  for (const [key, rates] of Object.entries(MODEL_RATES)) {
    const boundary =
      modelId.length === key.length || modelId[key.length] === "-";
...
export interface ModelCostEstimate {
  /** Canonical pricing key the usage was matched to. */
  model: string;
  tokens: {
    inputTokens: number;
    outputTokens: number;
...
export interface CostEstimate {
  ...
  totalUsd: number;
  /** Model IDs that carried usage but have no known list price. */
  unknownModels: string[];
  /** True when some token usage had no model attached at all. */
  hasUsageWithoutModel: boolean;
}
 ...
 */
export function estimateCost(events: FlightEvent[]): CostEstimate {
  const perModel = new Map<string, ModelCostEstimate>();
...
export const shortModelName = (id: string) => id.replace(/^claude-/, "");
