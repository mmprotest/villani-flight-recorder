...
export interface GraphCopyContext {
  eventCount: number;
  unknownCount: number;
  warningCount: number;
  hasSource: boolean;
  ...
  diffOk: boolean;
  gitStatus?: string;
  captured: CapturedRunStatusSummary;
  replayStatus: ReplayStatusSummary;
}

export function graphNodeTitle(nodeId: string): string {
  return (
    (
      {
        discover: "Discover",
...
export function graphNodeSubtitle(
  nodeId: string,
  ...
  switch (nodeId) {
    ...
    case "replay-output":
      if (replayStatus.status === "write_failed") return "Write failed";
      if (replayStatus.status === "render_failed") return "Render failed";
      if (
...
export function graphNodeBadge(
  nodeId: string,
  context: GraphCopyContext,
): string | undefined {
  if (nodeId === "commands") {
