...
export const truncateText = (value: unknown, maxChars = 20_000) => {
...
type PhaseStatus = "completed" | "warning" | "failed" | "pending";
...
const hasFailure = (events: FlightEvent[]) =>
  events.some((e) => e.type === "error" || (e.exitCode ?? 0) !== 0);
const statusText = (events: FlightEvent[]) =>
  hasFailure(events) ? "FAILED" : "REPLAY";
const taskTitle = (session: ParsedSession) =>
  session.events.find((e) => e.type === "user_message")?.summary ??
  session.events.find((e) => e.type === "user_message")?.title ??
  "Unknown task";
const sessionDuration = (session: ParsedSession) => {
...
function deriveReplayViewModel(
  session: ParsedSession,
  git: GitInfo | null,
): DashboardViewModel {
  ...
  const graph: GraphNodeViewModel[] = [
    {
      id: "discover",
      title: "Discover",
...
function metricCards(vm: DashboardViewModel) {
  const s = vm.session;
...
function eventStatus(e: FlightEvent): PhaseStatus {
  if (e.type === "error" || (e.exitCode ?? 0) !== 0) return "failed";
  if ((e.warnings?.length ?? 0) > 0 || e.type === "unknown") return "warning";
  return "completed";
}
function timeline(vm: DashboardViewModel) {
...
function executionGraph(vm: DashboardViewModel) {
  const statusClass = (a: PhaseStatus, b: PhaseStatus = a) =>
    a === "failed" || b === "failed"
      ? "fail"
      : a === "warning" || b === "warning"
        ? "warn"
...
function detailPanel(vm: DashboardViewModel) {
  const tabs = [
    ["Event Detail", ""],
