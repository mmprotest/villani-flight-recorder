...
const statusFileName = (line: string) => line.trim().split(/\s+/).pop() ?? "";
export function changedFilesFromEvents(events: FlightEvent[]) {
  ...
  for (const e of events) {
    ...
    for (const name of asStrings(raw.files)) files.add(name);
    const text =
...
export const changedFilesFromGit = (git: GitInfo | null) =>
  (git?.status ?? "")
    ...
    .map((l) => l.trim())
    .filter(Boolean);
const normalizeDiffBlock = (s: string) => s.trim().replace(/\r\n/g, "\n");
function dedupeDiffBlocks(blocks: string[]) {
...
export function diffFromEvents(events: FlightEvent[]) {
  ...
  return dedupeDiffBlocks(
    events
      .map(
        (e) =>
          e.diff ?? String(rawObj(e.raw).diff ?? rawObj(e.raw).patch ?? ""),
      )
      .filter(Boolean),
  ).join("\n\n");
}
