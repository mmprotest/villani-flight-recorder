...
import { TaskSegmentRecord } from "./sessionTypes.js";
const titleOf = (s: string, n: number) =>
  (s || `Task segment ${n}`).replace(/\s+/g, " ").trim().slice(0, 80) ||
  `Task segment ${n}`;
const repoOf = (e: FlightEvent) => (e.cwd ? path.resolve(e.cwd) : undefined);
export function segmentSession(
  sessionId: string,
  provider: any,
  events: FlightEvent[],
  repoIdsByRoot: Map<string, string>,
): TaskSegmentRecord[] {
  if (!events.length) return [];
  const starts: { i: number; reason: string }[] = [
  ...
  for (let i = 1; i < events.length; i++) {
    ...
    else if (gap > 20 * 60 * 1000) starts.push({ i, reason: "large time gap" });
  ...
  return starts.map((s, idx) => {
    ...
    const roots = [...new Set(slice.map(repoOf).filter(Boolean) as string[])];
    const changed = [
      ...new Set(slice.map((e) => e.path).filter(Boolean) as string[]),
    ];
    const firstPrompt =
      slice.find((e) => e.type === "user_message")?.summary ||
    ...
    return {
      ...
      failedCommandCount: slice.filter(
