...
export async function parseGeneric(provider, sessionPath) {
    ...
    for (const r of recs) {
        if (r.error) {
            ...
            warnings.push(w);
            events.push(event(`${provider}-${++n}`, provider, "error", "Invalid JSONL line", { raw: r.raw }, { warnings: [w] }));
            continue;
        }
        const o = obj(r.value);
        ...
        let type = "unknown";
        if (/user/i.test(typeName))
            type = "user_message";
        else if (/assistant/i.test(typeName))
            type = "assistant_message";
        else if (command)
            type = isTestCommand(command) ? "test_run" : "bash_command";
        else if (path && /read/i.test(typeName))
            type = "file_read";
        else if (path && /(edit|patch)/i.test(typeName))
            type = "file_edit";
        else if (path && /write/i.test(typeName))
            type = "file_write";
        const e = event(`${provider}-${++n}`, provider, type, "", r.value, {
    ...
    return finish(provider, sessionPath, events, warnings, recs.map((r) => r.value));
}
export function finish(provider, sessionPath, events, warnings, records) {
    const meta = records
        .map(obj)
