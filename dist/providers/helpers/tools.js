import { extractCommand, extractPath, isTestCommand, } from "../../normalize/events.js";
export function classifyTool(name, input) {
    ...
    if (["read", "view"].includes(lower))
        return { type: "file_read", path };
    if (["write"].includes(lower))
        return { type: "file_write", path };
    if (["edit", "multiedit", "notebookedit", "apply_patch", "patch"].includes(lower))
