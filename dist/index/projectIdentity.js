...
function decodeClaudeProjectPath(sourcePath) {
    if (!sourcePath)
        return undefined;
    ...
    const taskMatch = raw.match(/(?:^|-)tasks-([^-]+(?:-[^-]+)*?)(?:-sandbox|-repo|-workspace|-tmp|$)/i);
    if (taskMatch)
        return `tasks/${taskMatch[1]}`;
    return raw.replace(/-/g, "/");
}
export function meaningfulProjectName(projectPath) {
    if (!projectPath)
        return undefined;
    const segs = splitPath(projectPath);
    const task = segs.findIndex((s) => s.toLowerCase() === "tasks");
    if (task >= 0 && segs[task + 1])
        return segs[task + 1];
    for (let i = segs.length - 1; i >= 0; i--) {
        const s = segs[i];
        if (s && !weak.has(s.toLowerCase()))
            return s;
    }
    return segs.at(-1);
}
export function deriveProjectIdentity(input) {
    ...
    const projectName = meaningfulProjectName(candidates.find((c) => meaningfulProjectName(c)));
    const projectId = input.repoIds?.[0] ??
