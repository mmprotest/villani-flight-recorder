...
import { bottomCenter, GRAPH_HEIGHT, GRAPH_WIDTH, leftCenter, rightCenter, straightPath, topCenter, verticalElbowPath, } from "../graphGeometry.js";
const markerFill = {
...
export const executionGraph = (vm) => {
    ...
    const markers = ["completed", "running", "warning", "failed", "pending"]
        .map((s) => `<marker id="arrow-${s}" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="4" markerHeight="4" orient="auto"><path d="M 0 0 L 8 4 L 0 8 Z" fill="${markerFill[s]}"></path></marker>`)
        .join("");
