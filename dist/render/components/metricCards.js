...
export const metricCards = (vm) => {
    ...
    const factCards = isGitReplay
        ...
        : `<article><b>${escapeHtml(String(vm.rawEvents.length))}</b><span>events captured</span></article><article><b>${value(runner)}</b><span>provider</span></article><article><b>${value(model)}</b><span>${subvalue(model) || "model"}</span></article><article><b>${value(duration)}</b><span>duration</span></article>${optionalFacts}`;
