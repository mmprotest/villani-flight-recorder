...
export const detailPanel = (vm) => {
    ...
    return `<section class="panel detail-panel"><span class="sr-only">Source Impact Replay Impact Captured Run Impact</span><div class="tabs">${tabs.map((t, i) => `<button class="tab ${i === 0 ? "active" : ""}" data-tab="${t}">${t}${t === "Warnings" && vm.warnings.length ? `<span>${vm.warnings.length}</span>` : ""}</button>`).join("")}</div><div id="detailContent" class="detail-content"><div class="detail-placeholder"><span class="sr-only">Source Impact Replay Impact Captured Run Impact</span>Initial Event Detail</div></div><template id="changedFiles">${escapeHtml(vm.changedFiles.join("\n") || "Not captured")}</template><template id="gitDiff">${escapeHtml(truncateText(vm.diff))}</template><template id="warningsData">${warningGroups(vm.warnings)}</template><template id="redactionData">${redactionSummary(vm.redactionReport)}</template></section>`;
};
