...
function summary(events, git) {
    const filesRead = new Set(events.filter((e) => e.type === "file_read" && e.path).map((e) => e.path));
    const filesEdited = new Set(events
        .filter((e) => ["file_edit", "file_write", "file_delete"].includes(e.type) && e.path)
        .map((e) => e.path));
    const commands = events.filter((e) => e.command);
    const tests = events.filter((e) => e.type === "test_run");
    const failed = events.filter((e) => (e.exitCode ?? 0) !== 0 || e.type === "error");
    const depFiles = [...filesEdited].filter((f) => /(^|\/)(package.json|package-lock.json|pnpm-lock.yaml|yarn.lock|Cargo.toml|Cargo.lock|requirements.*\.txt|pyproject.toml|go.mod|go.sum)$/i.test(f ?? ""));
    const testFiles = [...filesEdited].filter((f) => /(test|spec|__tests__)/i.test(f ?? ""));
...
const sessionAny = (events) => events;
const icon = (name) => {
    const paths = {
        task: '<path d="M12 3l7 7-7 7-7-7 7-7z"/>',
        model: '<path d="M12 3l8 4.5v9L12 21l-8-4.5v-9L12 3z"/><path d="M12 12l8-4.5M12 12v9M12 12L4 7.5"/>',
        runner: '<circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M2 12h3M19 12h3M4.9 19.1L7 17M17 7l2.1-2.1"/>',
        ...
        shield: '<path d="M12 3l8 4v5c0 5-3.4 8-8 9-4.6-1-8-4-8-9V7l8-4z"/><path d="M9 12l2 2 4-4"/>',
        ...
        trophy: '<path d="M8 4h8v4a4 4 0 0 1-8 0V4z"/><path d="M8 6H4a4 4 0 0 0 4 4M16 6h4a4 4 0 0 1-4 4M12 12v4M8 20h8M10 16h4"/>',
    };
    return `<svg viewBox="0 0 24 24" aria-hidden="true">${paths[name] ?? paths.status}</svg>`;
};
function metric(label, value, sub = "", iconName = "status", cls = "") {
...
function statusOf(e, i, total) {
    if (e.type === "error" || (e.exitCode ?? 0) !== 0)
        return "failed";
    if (i === Math.max(0, total - 2))
        return "running";
    if (i === total - 1)
        return "pending";
    return "completed";
...
function taskTitle(session) {
    return (session.events.find((e) => e.type === "user_message")?.summary ??
...
function topBar(session, progress) {
...
function executionGraph(events) {
    const nodes = [
        "Discover",
        "Parse",
        "Normalize",
...
function detailTabs(session, git, warnings, report) {
