...
export function deriveExecutionGraph(input) {
    ...
    const replayStatus = input.replayStatus ??
        deriveReplayStatus({
            events,
            warnings,
    ...
    const parseSeverity = replayStatus.status === "parse_failed"
        ? "failed"
        : warnings.length === 0
            ? "none"
            : warnings.length <= 2
                ? "minor-warning"
                : "warning";
    const agentSeverity = captured.status === "not_applicable"
        ? "skipped"
        : events.length === 0
            ? "unavailable"
            : unknown > known
                ...
                : unknown > 0
                    ? "minor-warning"
                    : "none";
    const commandSeverity = captured.status === "not_applicable"
        ? "skipped"
    ...
    const nodes = [
        ...
        node("diff-capture", "Diff Capture", diffOk ? "none" : hasGit ? "minor-warning" : "unavailable", diffOk
            ? "diff available"
            : hasGit
                ? "no git diff captured"
                : "no git diff captured", "edit", diffOk ? undefined : "not captured"),
