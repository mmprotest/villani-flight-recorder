...
export function renderSessionBrowser(index, opts = {}) {
    const rows = index.sessions.map((s) => ({
        ...
        tokenLabel: s.tokenCount !== undefined ? formatTokenCount(s.tokenCount) : "",
        costUsd: s.costUsd,
        costLabel: "",
        isSubagent: false,
        subagentOf: "",
    ...
    // Claude Code subagent transcripts (<parent-uuid>/subagents/<name>.jsonl)
    // are indexed as separate sessions. Badge them and roll their token and
    // estimated-cost totals up into the parent session row.
    const byResolvedPath = new Map(rows
        .filter((r) => r.sourcePath)
        .map((r) => [path.resolve(r.sourcePath), r]));
    for (const r of rows) {
        const parentPath = r.sourcePath
            ? subagentParentPath(r.sourcePath)
            : undefined;
        if (!parentPath)
            continue;
        r.isSubagent = true;
        const parent = byResolvedPath.get(path.resolve(parentPath));
        if (parent && parent !== r) {
            r.subagentOf = parent.id;
    ...
    for (const r of rows) {
        ...
        if (typeof r.costUsd === "number")
            r.costLabel = `${formatUsd(r.costUsd)} est.`;
    }
