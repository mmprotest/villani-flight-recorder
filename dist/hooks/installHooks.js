...
export async function appendHook(provider, input, cwd = process.cwd()) {
    const lines = input.split(/\r?\n/).filter((l) => l.trim());
    if (!lines.length)
        throw new Error("No hook JSON was provided on stdin.");
    ...
    const records = lines.map((line, i) => {
        try {
            ...
            throw new Error(`Invalid JSON on stdin line ${i + 1}: ${e instanceof Error ? e.message : String(e)}`);
        }
    });
    const sid = safeSegment(String(records.find((r) => r.session_id)?.session_id ??
    ...
    await fs.appendFile(file, records
